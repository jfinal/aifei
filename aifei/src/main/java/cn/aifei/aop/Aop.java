 ...
 *
 *  4：AopKit.get().setSingleton(...) 用于配置默认是否为单例
 *
 ...
 *
 *  2：被注入对象默认是 singleton 单例，可以通过 AopKit.get().setSingleton(false) 配置默认不为单例
 *
...
public class Aop {
    ...
    /* 通过 AopKit.get().getAopFactory().inject(...) 可调用如下方法，不直接开放出来
